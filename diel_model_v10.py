...
• NO air-water CO₂ gas exchange (closed-system scenario)
  — iso_gas term removed from isotope mass balance
  — k_gas_iso parameter removed from optimisation

Usage:
    python diel_model_v10.py                  # uses defaults
    python diel_model_v10.py path/to/data.csv # custom CSV
"""
...
OUTPUT_DIR    = 'output\\summer\\output_v10'
...
def main():
    print("=" * 70)
    print("DIEL 13C-DIC MODEL v10 — No Air-Water CO₂ Exchange (Closed System)")
    print("=" * 70)
    ...
    df_raw = df_raw.rename(columns={
        ...
        'Sodium (mg L-1)': 'Na_mgL', 'Na (mg L-1)': 'Na_mgL',
    ...
    # v10: gas exchange is disabled — flux is computed for diagnostic output only
    # but does NOT feed back into the isotope mass balance
    flux_mmol  = kco2_b * (CO2_mol - CO2_eq_atm) * 1e6   # diagnostic only

    # ── Model function ──────────────────────────────────────────────────
    def run_model(pvec):
        GPP_max, Ik, Q10_gpp, ER_base, Q10_er, pf, d13c_org = pvec
        ...
        for i in range(1, n):
            ...
            # v10: NO air-water CO₂ exchange — iso_gas = 0
            iso_gas = 0.0
    ...
    plabels = ['GPP_max (mgO2/L/h)', 'Ik (W/m²)', 'Q10_GPP',
               'ER_base (mgO2/L/h)', 'Q10_ER', 'PF (‰)', 'δ13C_org (‰)']
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'diel_v10_results.csv')
    ...
    ax.set_title('Air-Water CO₂ Exchange (DIAGNOSTIC ONLY — not active in v10)',
    ...
    plt.tight_layout()
    for fmt in ['svg', 'png']:
        fig.savefig(os.path.join(OUTPUT_DIR, f'diel_v10_results.{fmt}'),
