...
def create_phreeqc(output_dir):
    ...
    db_path = os.path.join(output_dir, 'phreeqc_v8.dat')
...
def main():
    print("=" * 70)
    print("DIEL 13C-DIC MODEL v8 — Measured Ca/Na/Cl, PAR-GPP, Bayesian Bounds")
    print("=" * 70)
    ...
    def objective(pvec):
        ...
        rmse_gpp = np.sqrt(np.mean((obs_gpp[gpp_mask] - mod_gpp[gpp_mask])**2)) \
                   if gpp_mask.sum() > 0 else 0

        w_gpp = 0.05   # normalise GPP to d13C scale
    ...
    bounds = [
        ...
        (20.0, 500.0),   # Ik       (W m⁻²)
        (1.2, 3.5),      # Q10_gpp
        (-15.0, -5.0),   # ER_base  (mg O₂ L⁻¹ h⁻¹, negative)
        (-30.0, -10.0),  # PF       (‰)
        (-34.0, -22.0),  # δ¹³C_org (‰)
        (0.3, 2.5),      # k_gas_iso
    ]
    ...
    plabels = ['GPP_max (mgO2/L/h)', 'Ik (W/m²)', 'Q10_GPP',
               'ER_base (mgO2/L/h)', 'PF (‰)', 'δ13C_org (‰)', 'k_gas_iso']
    ...
    print(f"\n  {'Time':>16s} {'d13C_o':>7s} {'d13C_m':>7s} {'Δ':>6s}"
          f"  {'GPP_o':>6s} {'GPP_m':>6s} {'Δ':>6s}"
          f"  {'SI_Cc':>6s} {'Ca':>5s} {'Na':>5s} {'Cl':>5s}")
    for i in range(len(obs_d13c)):
        ...
        ca_i = df_obs['Ca_mgL'].iloc[i]
        na_i = df_obs['Na_mgL'].iloc[i]
        cl_i = df_obs['Cl_mgL'].iloc[i]
        print(f"  {ts:>16s} {obs_d13c[i]:7.3f} {mod_d13c[i]:7.3f}"
              f" {obs_d13c[i]-mod_d13c[i]:+6.3f}"
              f"  {obs_gpp[i]:6.2f} {mod_gpp[i]:6.2f}"
              f" {obs_gpp[i]-mod_gpp[i]:+6.2f}"
              f"  {si_i:6.3f} {ca_i:5.1f} {na_i:5.3f} {cl_i:5.3f}")
    ...
    csv_path = os.path.join(OUTPUT_DIR, 'diel_v8_results.csv')
    ...
    ax.plot(tt, GPP_final, '-', color=c_gpp, lw=2,
            label=f'Model (max={bx[0]:.1f}, Ik={bx[1]:.0f}, Q10={bx[2]:.2f})')
    ...
    # 5 ─ ER
    ax = axes[4]
    ax.axhline(bx[3], color=c_er, lw=2,
               label=f'Model ({bx[3]:.2f} mg O₂/L/h)')
    ...
    plt.tight_layout()
    for fmt in ['svg', 'png']:
        fig.savefig(os.path.join(OUTPUT_DIR, f'diel_v8_results.{fmt}'),
